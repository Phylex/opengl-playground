...
void Polyline::init_gl_context() {
	GLCall(glBindVertexArray(vao));
	GLCall(glBindBuffer(GL_ARRAY_BUFFER, vbo));
}

Polyline::Polyline(std::vector<glm::vec2> &in_points, ShaderProgram *in_shader) {
	points = in_points;
	shader = in_shader;
...
void Polyline::add_points(glm::vec2 point) {
...
void Polyline::add_points(std::vector<glm::vec2>& add_points) {
...
void Polyline::write_points_to_gl_array() {
